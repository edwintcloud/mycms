...
function show_forum_rank($posts, $level, $image=1, $text=1) {
		...
		if ($level > 101) {
	      ...
	      for ($i = 0; $i < count($forum_mod_rank_cache) && !$res; $i++) {
					if ($level == $forum_mod_rank_cache[$i]['rank_apply']) {
						$res = ($text ? $forum_mod_rank_cache[$i]['rank_title']."<br />\n" : "").($image ? "<img src='".RANKS.$forum_mod_rank_cache[$i]['rank_image']."' alt='' style='border:0' />" : "");
					}
				}
			}
		}	
			...
			if (is_array($forum_rank_cache) && count($forum_rank_cache)) {
				for ($i = 0; $i < count($forum_rank_cache); $i++) {
					if ($posts >= $forum_rank_cache[$i]['rank_posts']) {
						$res = ($text ? $forum_rank_cache[$i]['rank_title']."<br />\n" : "").($image ? "<img src='".RANKS.$forum_rank_cache[$i]['rank_image']."' alt='' style='border:0' />" : "");
		      }
		    }
				if (!$res) {
					$res = ($text ? $forum_rank_cache[0]['rank_title']."<br />\n" : "").($image ? "<img src='".RANKS.$forum_rank_cache[0]['rank_image']."' alt='' style='border:0' />" : "");
	      }
			}
		}
	}
