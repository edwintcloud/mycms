...
define("SCRIPT_URL_EXT", "/".substr(preg_replace("#(&amp;|\?)(s_action=edit&amp;shout_id=)([0-9]+)#s", "", $start_page), 1));

// Calculate current true script url
$script_url = explode("/", $_SERVER['PHP_SELF']); // .(FUSION_QUERY ? "?".FUSION_QUERY : "")
$url_count = count($script_url);
$base_url_count = substr_count(BASEDIR, "/") + 1;
$start_page = "";
...
define("SCRIPT_URL", "/".substr(preg_replace("#(&amp;|\?)(s_action=edit&amp;shout_id=)([0-9]+)#s", "", $start_page), 1));
...
if(!defined("ADMIN_PANEL")){
	...
	$tableFields = dbquery("SELECT * FROM ".DB_PANELS);
