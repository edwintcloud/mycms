...
if (isset($_POST['previewchanges']) || isset($_POST['update_poll_title']) || isset($_POST['update_poll_option']) || isset($_POST['delete_poll_option']) || isset($_POST['add_poll_option'])) {
	...
	$edit_reason = isset($_POST['edit_reason']) ? trim(stripinput(censorwords($_POST['edit_reason']))) : "";
	...
	if (isset($_POST['previewchanges'])) {
		...
		if($fb4['post_icons']){ $post_icon = (isset($_POST['post_icon']) ? stripinput($_POST['post_icon']) : ""); }
		...
		if($subject) echo "<tr>\n<td colspan='2' class='tbl2'><strong>".$subject."</strong></td>\n</tr>\n";
		...
		echo "<hr />\n".$locale['427']."<a href='../profile.php?lookup=".$userdata['user_id']."'>".$userdata['user_name']."</a>".$locale['428'].showdate("forumdate", time());
		if($edit_reason) echo " (".$locale['fb600']." ".$edit_reason.")";
		echo "</td>\n";
		echo "</tr>\n</table>\n";
		closetable();
	}
...
if (isset($_POST['savechanges'])) {
	if (isset($_POST['delete'])) {
		...
		if (dbrows($result)) {
			...
			while($attach = dbarray($result)){
				...
				$result2 = dbquery("DELETE FROM ".DB_FORUM_ATTACHMENTS." WHERE attach_id='".$_GET['attach_id']."'");
			}
		...
		$post_showsig = (isNum($_POST['post_showsig']) ? $_POST['post_showsig'] : 0);
		if (iMEMBER) {
			if ($message != "") {
				$result = dbquery("UPDATE ".DB_POSTS." SET post_message='$message', post_smileys='$smileys', ".(isset($_POST['hide_edit']) && $_POST['hide_edit'] ? "" : "post_edituser='".$userdata['user_id']."', post_edittime='".time()."', ")."post_showsig='$post_showsig' WHERE post_id='".$_GET['post_id']."'");
				...
				$r = dbquery("select * from ".$db_prefix."fb_posts where post_id='".$_GET['post_id']."'");
				if(dbrows($r)){
					$query = dbquery("update ".$db_prefix."fb_posts set post_editreason='$reason', post_icon='$icon' where post_id='".$_GET['post_id']."'");
				} else {
					$query = dbquery("insert into ".$db_prefix."fb_posts (post_id, post_editreason, post_icon) VALUES('".$_GET['post_id']."', '$reason', '$icon')");
				}
				...
				if ($pdata['first_post'] == $_GET['post_id']){
					if($fb4['announce_enable'] && checkgroup($fb4['announce_create'])){
						$announcement = (isset($_POST['make_announcement']) ? $_POST['make_announcement'] : "0");
						$a_res = dbquery("select * from ".DB_PREFIX."fb_threads where thread_id='".$_GET['thread_id']."'");
						if(dbrows($a_res)){
							$a_data = dbarray($a_res);
							$query = dbquery("update ".DB_PREFIX."fb_threads set thread_announcement='$announcement' where thread_id='".$_GET['thread_id']."'");
						} else {
							$query = dbquery("insert into ".DB_PREFIX."fb_threads (thread_id, thread_announcement) values('".$_GET['thread_id']."', '$announcement')");
						}
					}
				}
	...
	if(dbrows($r)){
		...
		$post_icon = $d['post_icon'];
	} else {
		$edit_reason = "";
		$post_icon = "page_white.png";
	}
	$sig_checked = ($pdata['post_showsig'] ? " checked='checked'" : "");
	opentable($locale['408']);
	if(!isset($_POST['previewchanges'])) renderPostNav($_GET['action'], $announcementCheck);
	...
	echo "</td>\n";
	echo "</tr>\n<tr>\n";
	echo "<td class='tbl2' valign='top'>".$locale['fb600']."</td>\n";
    echo "<td class='tbl1'><input type='text' name='edit_reason' class='textbox' style='width:250px;' value='$edit_reason'>";
	echo "</td>\n";
    echo "</tr>\n<tr>\n";
	echo "<td width='145' class='tbl2'>&nbsp;</td>\n";
	echo "<td class='tbl1'>".display_bbcodes("99%", "message")."</td>\n";
	echo "</tr>\n<tr>\n";
	if ($pdata['first_post'] == $_GET['post_id'] && $fb4['post_icons']) {
		echo "<td valign='top' width='145' class='tbl2'>".$locale['fb613']."</td>\n";
		echo "<td class='tbl1'>\n";
		echo "<label><input type='radio' name='post_icon' value=''> ".$locale['fb614']."</label><br />\n";
		$postIcons = makefilelist(INFUSIONS."fusionboard4/images/post_icons/", ".|..|index.php|Thumbs.db");
		foreach($postIcons as $icon){
			echo "<label><input type='radio' name='post_icon' value='$icon'".($icon==$post_icon ? " CHECKED" : "")."> <img src='".INFUSIONS."fusionboard4/images/post_icons/$icon' alt='$icon'></label>&emsp;";
		}
		echo "</td>\n</tr>\n<tr>\n";
	}
	...
	if ($pdata['first_post'] == $_GET['post_id']){
		if($fb4['announce_enable'] && checkgroup($fb4['announce_create'])){
			...
			if(dbrows($a_res)){
				$a_data = dbarray($a_res);
				$announce = ($a_data['thread_announcement'] ? " CHECKED" : "");;
			} else {
				$announce = "";
			}
			echo "<label><input type='checkbox' name='make_announcement' value='1'$announce /> ".$locale['fb903']."</label><br />\n";
		}
	}
	if(iMOD || iADMIN) echo "<label><input type='checkbox' name='hide_edit' value='1'> ".$locale['fb603']."</label><br />\n";
	...
	if ($fdata['forum_attach'] && checkgroup($fdata['forum_attach'])) {
		...
		<script>
			...
			var multi_selector = new MultiSelector( document.getElementById( \"files_list\" ), ".($fb4['max_attach'] - $counter)." );
