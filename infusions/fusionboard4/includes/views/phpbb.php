...
while ($data = dbarray($result)) {
	...
	<tr>\n<td valign='top' class='".( ($current_row & 1) ? 'tbl1' : 'tbl2' )."' style='padding:10px;'>
	...
	echo "<div style='padding-bottom:2px;font-size:14px;'>$ficon".($announcementCheck ? $locale['fb902'] : "")."<strong>
    <!--forum_thread_title-->".($current_row > 1 ? "RE: " : "").$fdata['thread_subject']."</strong>";
	if (iMOD) { echo "&nbsp;<input type='checkbox' name='delete_post[]' value='".$data['post_id']."' style='vertical-align:bottom;' />\n"; }
	echo "</div>
	<span class='small'>by: <a href='".BASEDIR."profile.php?lookup=".$data['user_id']."'>".showLabel($data['user_id'], false, "post")."</a>, ".timePassed($data['post_datestamp']);
	...
	if ($data['user_avatar'] && file_exists(IMAGES."avatars/".$data['user_avatar'])) {
		...
		if($fb4['no_avatar']) echo "<img src='".IMAGES."noav.gif' alt='".$locale['567']."' /><br />\n";
	}
	...
	if(((dbrows(dbquery("select * from ".DB_PREFIX."fb_rate where rate_post='".$data['post_id']."'")) || (iMEMBER && $userdata['user_id'] !== $data['user_id']))  && $fb4['show_ratings']) || (iMEMBER & ($can_post || $can_reply))){
		echo "<tr><td class='".( ($current_row & 1) ? 'tbl1' : 'tbl2' )."' style='padding:6px;'>\n";
		
		if($fb4['show_ratings']){
			postRatings($data['post_id']);
			echo "<div align='right'>\n";
			if(iMEMBER && $userdata['user_id'] !== $data['user_id']){
				echo "<span id='rb_".$data['post_id']."'>";
