...
while ($data = dbarray($result)) {
	...
	echo "<tr>\n<td class='tbl2 small' style='width:140px; padding:6px;' nowrap='nowrap'>".timePassed($data['post_datestamp'])."</td>\n";
	echo "<td class='tbl2' style='padding:6px;'><div style='float:right' class='small'><a href='#post_".$data['post_id']."' name='post_".$data['post_id']."' id='post_".$data['post_id']."'>#".($current_row+$_GET['rowstart'])."</a></div>\n";
	if($fb4['post_icons'] && $current_row == "1"){
	...
	echo "$ficon".($current_row > 1 ? "RE: " : "").($announcementCheck ? $locale['fb902'] : "")."
	<strong><!--forum_thread_title-->".trimlink($fdata['thread_subject'], 40);
	...
	echo "</td>\n<td valign='top' class='tbl1' style='padding:7px;'>\n";
