...
/*
	...
	ianunruh@gmail.com
*/
if(!defined("IN_FUSION")) redirect("../usercp.php");
...
if (isset($_POST['save_options'])) {
	...
	redirect(FUSION_SELF."?section=pm&amp;folder=options");
}
...
if (isset($_GET['msg_id']) && isnum($_GET['msg_id'])) {
	if (isset($_POST['save'])) {
		...
		redirect(FUSION_SELF."?section=pm&amp;folder=archive".($error ? "&error=$error" : ""));
		...
		redirect(FUSION_SELF."?section=pm&amp;folder=".$_GET['folder']);
	}
}
...
if ($msg_ids && $check_count > 0) {
	...
	redirect(FUSION_SELF."?section=pm&amp;folder=".$_GET['folder'].($error ? "&error=$error" : ""));
}
...
if (isset($_POST['send_message'])) {
	...
	if ($subject == "" || $message == "") { redirect(FUSION_SELF."?section=pm&amp;folder=inbox"); }
	...
	if (iADMIN && isset($_POST['chk_sendtoall']) && isnum($_POST['msg_to_group'])) {
		...
		if ($msg_to_group == "101" || $msg_to_group == "102" || $msg_to_group == "103") {
			...
			if (dbrows($result)) {
				...
				redirect(FUSION_SELF."?section=pm&amp;folder=inbox");
			}
		} else {
			$result = dbquery(
				"SELECT u.user_id, u.user_name, u.user_email, mo.pm_email_notify FROM ".DB_USERS." u
	...
	redirect(FUSION_SELF."?section=pm&amp;folder=inbox".($error ? "&error=$error" : ""));
}
...
if (!isset($_GET['msg_send']) && !isset($_GET['msg_read']) && $_GET['folder'] != "options") {
	...
	openSection($locale['400']);
	if ($total_rows) echo "<form name='pm_form' method='post' action='".FUSION_SELF."?section=pm&amp;folder=".$_GET['folder']."'>\n";
	echo "<table cellpadding='0' cellspacing='0' width='100%'>\n";
	echo "<tr>\n<td align='left' width='100%' class='tbl'><a href='".FUSION_SELF."?section=pm&amp;msg_send=0'>".$locale['401']."</a></td>\n";
	echo "<td width='1%' class='tbl' style='white-space:nowrap;font-weight:".($_GET['folder']=="inbox"?"bold":"normal")."'><a href='".FUSION_SELF."?section=pm&amp;folder=inbox'>".$locale['402']." [".$bdata['inbox_total']."/".$msg_settings['pm_inbox']."]</a></td>\n";
	echo "<td width='1%' class='tbl' style='white-space:nowrap;font-weight:".($_GET['folder']=="outbox"?"bold":"normal")."'><a href='".FUSION_SELF."?section=pm&amp;folder=outbox'>".$locale['403']." [".$bdata['outbox_total']."/".$msg_settings['pm_sentbox']."]</a></td>\n";
	echo "<td width='1%' class='tbl' style='white-space:nowrap;font-weight:".($_GET['folder']=="archive"?"bold":"normal")."'><a href='".FUSION_SELF."?section=pm&amp;folder=archive'>".$locale['404']." [".$bdata['archive_total']."/".$msg_settings['pm_savebox']."]</a></td>\n";
	echo "<td width='1%' class='tbl' style='white-space:nowrap;font-weight:".($_GET['folder']=="options"?"bold":"normal")."'><a href='".FUSION_SELF."?section=pm&amp;folder=options'>".$locale['425']."</a></td>\n";
	echo "</tr>\n</table>\n";
	if ($total_rows) {
		...
		while ($data = dbarray($result)) {
			...
			echo "<a href='".FUSION_SELF."?section=pm&amp;folder=".$_GET['folder']."&amp;msg_read=".$data['message_id']."'>".$message_subject."</a></td>\n";
	...
	echo "}\n}\n}\n</script>\n";
	if ($total_rows > 20) echo "<div align='center' style='margin-top:5px;'>\n".makepagenav($_GET['rowstart'], 20, $total_rows, 3, FUSION_SELF."?section=pm&amp;folder=".$_GET['folder']."&amp;")."\n</div>\n";
	closeSection();
	...
	echo "</tr>\n</table>\n";
	tablebreak();
	echo "<form name='options_form' method='post' action='".FUSION_SELF."?section=pm&amp;folder=options'>\n";
	...
	if (dbrows($result)) {
		...
		openSection($locale['431']);
		echo "<form name='pm_form' method='post' action='".FUSION_SELF."?section=pm&amp;folder=".$_GET['folder']."&amp;msg_send=".$data['user_id']."&amp;msg_id=".$data['message_id']."'>\n";
		echo "<table cellpadding='0' cellspacing='1' width='100%' class='tbl-border'>\n<tr>\n";
		...
		echo "</table>\n";
		echo "<table cellpadding='0' cellspacing='0' width='100%'>\n";
		echo "<tr>\n<td colspan='2' class='tbl'><a href='".FUSION_SELF."?section=pm&amp;folder=".$_GET['folder']."'>".$locale['432']."</a></td>\n";
	...
	openSection($locale['420']);
	echo "<form name='inputform' method='post' action='".FUSION_SELF."?section=pm&amp;msg_send=0' onsubmit=\"return ValidateForm(this)\">\n";
	echo "<table cellpadding='0' cellspacing='1' width='100%' class='tbl-border'>\n";
	...
	echo "<tr>\n<td class='tbl'><a href='".FUSION_SELF."?section=pm&amp;folder=inbox'>".$locale['435']."</a></td>\n";
...
function openSection($title){
	echo "<tr>\n<td class='tbl2 navtitle'>".$title."</td>\n</tr>\n";
	echo "<tr>\n<td class='tbl1'>\n";
}
