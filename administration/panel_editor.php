...
if (isset($_POST['save'])) {
	...
	$panel_url_list_exclude = isset($_POST['panel_url_list_exclude']) ? $_POST['panel_url_list_exclude'] : "0"; ///#
	...
	if (isset($_GET['panel_id']) && isnum($_GET['panel_id'])) {
		...
		If (($panel_side == "2" || $panel_side == "3") && $panel_display == "0" && $panel_url_list !== "") { $error .= $locale['cpac_014']."<br />"; } // 0.26 ///
		...
		if (!$error) {
			$result = dbquery("UPDATE ".DB_PANELS." SET panel_name='$panel_name', panel_url_list='$panel_url_list', panel_url_list_exclude='$panel_url_list_exclude', panel_filename='$panel_filename', panel_content='$panel_content', panel_access='$panel_access', panel_display='$panel_display' WHERE panel_id='".$_GET['panel_id']."'"); ///
		}
	...
	if (isset($_POST['preview'])) {
		$panel_name = stripinput($_POST['panel_name']);
		$panel_url_list = stripinput($_POST['panel_url_list']); ///
		////$panel_url_list_exclude = stripinput($_POST['panel_url_list_exclude']); /// #
		$pos1_check = ($_POST['panel_url_list_exclude']=="1" ? " checked='checked'" : ""); ///#
		$pos2_check = ($_POST['panel_url_list_exclude']=="0" ? " checked='checked'" : ""); ///#
	...
	if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['panel_id']) && isnum($_GET['panel_id']))) {
		...
		if (dbrows($result)) {
			...
			$pos1_check = ($data['panel_url_list_exclude']=="1" ? " checked='checked'" : ""); ///#
			$pos2_check = ($data['panel_url_list_exclude']=="0" ? " checked='checked'" : ""); ///#
	...
	// URL list, exclude or include ///
	echo "<tr>\n";
	echo "<td valign='top' class='tbl'>".$locale['cpac_010']."<br /><span class='small2'><i>".$locale['cpac_011']."<br />/news.php<br />/forum/index.php</i></span></td>\n";
	echo "<td width='200' valign='top' class='tbl'><textarea name='panel_url_list' cols='50' rows='5' class='textbox' style='width:200px;'>".$panel_url_list."</textarea></td>\n";
	echo "<td valign='top' class='tbl'><label><input type='radio' name='panel_url_list_exclude' value='1'".$pos1_check." /> ".$locale['cpac_012']."</label><br />\n";
	echo "<label><input type='radio' name='panel_url_list_exclude' value='0'".$pos2_check." /> ".$locale['cpac_013']."</label><br />\n</td>\n";
	echo "</tr>\n";
	...
	if ((!isset($_COOKIE[COOKIE_PREFIX.'admin']) || md5($_COOKIE[COOKIE_PREFIX.'admin']) != $userdata['user_admin_password']) && md5(md5($admin_password)) != $userdata['user_admin_password']) {
		...
		echo "<td colspan='2' class='tbl'><input type='password' name='admin_password' value='".$admin_password."' class='textbox' style='width:150px;' /></td>\n";
		echo "</tr>\n";
	}
	if (!isset($_GET['panel_id']) || !isnum($_GET['panel_id'])) {
