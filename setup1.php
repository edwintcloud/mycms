...
if (isset($_POST['step']) && $_POST['step'] == "2") {
	...
	echo "<tr>\n<td class='tbl1'>Configuration</td>\n";
	echo "<td class='tbl1' style='text-align:right'>".(is_writable("Configuration") ? "<span class='passed'>".$locale['023']."</span>" : "<span class='failed'>".$locale['024']."</span>")."</td>\n</tr>\n";
	echo "</table><br><br>\n";
	if ($write_check) {
		echo $locale['021']."\n";
		echo "</td>\n</tr>\n<tr>\n<td class='tbl2' style='text-align:center'>\n";
...
if (isset($_POST['step']) && $_POST['step'] == "3") {
	...
	echo "<td class='tbl1' style='text-align:right'><input type='text' value='arcsite_' name='db_prefix' class='textbox' style='width:200px'></td>\n</tr>\n";
	echo "<tr>\n<td class='tbl1'>".$locale['036']."</td>\n";
	echo "<td class='tbl1' style='text-align:right'><input type='text' value='arcemu' name='emu_type' class='textbox' style='width:200px'><br /> *type either arcemu or mangos</td>\n</tr>\n";
	echo "</table>\n";
	echo "</td>\n</tr>\n<tr>\n<td class='tbl2' style='text-align:center'>\n";
	echo "<input type='hidden' name='localeset' value='".stripinput($_POST['localeset'])."'>\n";
...
if (isset($_POST['step']) && $_POST['step'] == "4") {
	...
	$emu_type = stripinput(trim($_POST['emu_type']));
	if ($link = dbconnect($db_host, $db_user, $db_pass, $db_name)) {
		...
		$config2 = "<?php\n";
		$config2 .= "/*\n";
		$config2 .= "This is the Main Configuration File! Any file using the\n";
		$config2 .= "maincore as an include/require can access these settings.\n";
		$config2 .= "\n";
		$config2 .= "Syntax:\n";
		$config2 .= "Configuration::Set('nameofsetting','setting')\n";
		$config2 .= "Configuration::Get(''nameofsetting')\n";
		$config2 .= "Thank Kirth from MMOwned for giving me this idea :P ..\n";
		$config2 .= "and his configuration class.\n";
		$config2 .= "\n";
		$config2 .= "Note: Do Not Change the first field in the Set unless\n";
		$config2 .= "you plan of modifying all the sites including that variable\n";
		$config2 .= "*/\n"."\n";
		$config2 .= "//Pull the Configuration Class module -MANDATORY\n";
		$config2 .= "require_once \"class.php\";  //DON'T TOUCH IT\n";
		$config2 .= "//DO NOT CHANGE ABOVE\n"."\n";
		$config2 .= "//Website Database Configuration\n";
		$config2 .= "Configuration::Set('website.db.host','".$db_host."');\n";
		$config2 .= "Configuration::Set('website.db.user','".$db_user."');\n";
		$config2 .= "Configuration::Set('website.db.pass','".$db_pass."');\n";
		$config2 .= "Configuration::Set('website.db.name','".$db_name."');\n"."\n";
		$config2 .= "//Emulator Setting\n";
		if($emu_type == "arcemu"){
		$config2 .= "Configuration::Set('emulator',1);\n"; }
		elseif($emu_type == "mangos"){
		$config2 .= "Configuration::Set('emulator',2);\n"; }
		elseif($emu_type != "mangos" || $emu_type != "arcemu") { $fail = true; $fail2 = true; }
		$config2 .= "?>";
		$temp = fopen("config.php","w");
		$temp2 = fopen("Configuration/mainconfig.php","w");
		if (fwrite($temp, $config) && fwrite($temp2, $config2)) {
			fclose($temp);
			fclose($temp2);
			$fail = false;
			...
			if (!$fail) {
				...
				if($fail2) {
				echo "<strong>".$locale['043']."</strong> ".$locale['050']."<br><br>\n"; }
				$success = false;
			}
		} else {
			echo "<br>\n".$locale['040']."<br><br>\n";
